...
bfd_check_format_matches(bfd *abfd, bfd_format format, char ***matching)
{
  ...
  if (!bfd_read_p(abfd) || (unsigned int)abfd->format >= (unsigned int)bfd_type_end)
  {
 ...
 target.  */
  ...
  for (target = bfd_target_vector; *target != NULL; target++)
  {
    ...
    if (*target == &binary_vec || (!abfd->target_defaulted && *target == save_targ))
      continue;
 ...
 _bfd_check_format might have this problem.  */
    ...
    if (temp)
    {
      ...
      if (abfd->format != bfd_archive || (bfd_has_map(abfd) && bfd_get_error() != bfd_error_wrong_object_format))
      {
  ...
  if (match_count > 1)
  {
    ...
    while ((right_targ = *assoc++) != NULL)
    {
      ...
      while (--i >= 0)
        if (matching_vector[i] == right_targ && right_targ->match_priority <= best_match)
          break;
...
{
  if (((int)format < (int)bfd_unknown) || ((int)format >= (int)bfd_type_end))
    return "invalid";

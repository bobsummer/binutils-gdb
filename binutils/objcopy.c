...
#define PARSE_FLAG(fname, fval)                                        \
  else if (len == sizeof fname - 1 && strncasecmp(fname, s, len) == 0) \
      ret |= fval

#define PARSE_OTHER(fname, fval)                                                \
  else if (len >= sizeof fname && strncasecmp(fname, s, sizeof fname - 1) == 0) \
  ...
  for (p = change_sections; p != NULL; p = p->next)
  {
    if (add)
    {
      if (strcmp(p->pattern, name) == 0)
      {
        ...
        if (((p->context & SECTION_CONTEXT_REMOVE) && (context & SECTION_CONTEXT_COPY)) || ((context & SECTION_CONTEXT_REMOVE) && (p->context & SECTION_CONTEXT_COPY)))
          fatal(_("error: %s both copied and removed"), name);

        if (((p->context & SECTION_CONTEXT_SET_VMA) && (context & SECTION_CONTEXT_ALTER_VMA)) || ((context & SECTION_CONTEXT_SET_VMA) && (context & SECTION_CONTEXT_ALTER_VMA)))
          fatal(_("error: %s both sets and alters VMA"), name);

        if (((p->context & SECTION_CONTEXT_SET_LMA) && (context & SECTION_CONTEXT_ALTER_LMA)) || ((context & SECTION_CONTEXT_SET_LMA) && (context & SECTION_CONTEXT_ALTER_LMA)))
...
add_specific_symbols(const char *filename, htab_t htab)
{
 ...
 then add it to the list.  */
    for (name = line; IS_WHITESPACE(*name); name++)
      ;
    for (name_end = name;
         (!IS_WHITESPACE(*name_end)) && (!IS_LINE_TERMINATOR(*name_end));
         name_end++)
      ;
...
group_signature(asection *group)
{
  ...
  if (ghdr->sh_link == elf_onesymtab(abfd))
  {
    ...
    if (ghdr->sh_info > 0 && ghdr->sh_info < symhdr->sh_size / bed->s->sizeof_sym)
...
{
  if (merge_notes && bfd_get_flavour(abfd) == bfd_target_elf_flavour && elf_section_data(sec)->this_hdr.sh_type == SHT_NOTE
...
is_strip_section_1(bfd *abfd ATTRIBUTE_UNUSED, asection *sec)
{
  ...
  {
    if (strip_symbols == STRIP_DEBUG || strip_symbols == STRIP_UNNEEDED || strip_symbols == STRIP_ALL || discard_locals == LOCALS_ALL || convert_debugging)
    {
...
is_strip_section(bfd *abfd ATTRIBUTE_UNUSED, asection *sec)
{
 ...
 strip the group section too.  */
    gname = gsym->name;
    if ((strip_symbols == STRIP_ALL && !is_specified_symbol(gname, keep_specific_htab)) || is_specified_symbol(gname, strip_specific_htab))
      return TRUE;
               ...
               asymbol **isyms, long symcount)
{
  ...
  for (; src_count < symcount; src_count++)
  {
    ...
    if (htab_elements(redefine_specific_htab) || section_rename_list)
    {
      ...
      if (new_name == name && (flags & BSF_SECTION_SYM) != 0)
    ...
    rem_leading_char =
        (name[0] == bfd_get_symbol_leading_char(abfd)) && (change_leading_char || (remove_leading_char && ((flags & (BSF_GLOBAL | BSF_WEAK)) != 0 || undefined || bfd_is_com_section(bfd_get_section(sym)))));

    /* Check if we will add a new leading character.  */
    add_leading_char =
        change_leading_char && (bfd_get_symbol_leading_char(obfd) != '\0') && (bfd_get_symbol_leading_char(abfd) == '\0' || (name[0] == bfd_get_symbol_leading_char(abfd)));
    ...
    if (add_leading_char || prefix_symbols_string)
    {
      ...
      ptr = n = (char *)xmalloc(1 + strlen(prefix_symbols_string) + strlen(name) + 1);
      if (add_leading_char)
    ...
    if (strip_symbols == STRIP_ALL)
      keep = FALSE;
             ...
             || ((flags & BSF_SECTION_SYM) != 0 && ((*bfd_get_section(sym)->symbol_ptr_ptr)->flags & BSF_KEEP) != 0))
    {
      keep = TRUE;
      used_in_reloc = TRUE;
    }
    else if (relocatable /* Relocatable file.  */
             && ((flags & (BSF_GLOBAL | BSF_WEAK)) != 0 || bfd_is_com_section(bfd_get_section(sym))))
      keep = TRUE;
      ...
      keep = TRUE;
             ...
             || (flags & BSF_WEAK) != 0 || undefined || bfd_is_com_section(bfd_get_section(sym)))
      keep = strip_symbols != STRIP_UNNEEDED;
    else if ((flags & BSF_DEBUGGING) != 0) /* Debugging symbol.  */
      keep = (strip_symbols != STRIP_DEBUG && strip_symbols != STRIP_UNNEEDED && !convert_debugging);
    else if (bfd_coff_get_comdat_section(abfd, bfd_get_section(sym)))
      /* COMDAT sections store special information in local
         symbols, so we cannot risk stripping any of them.  */
      keep = TRUE;
    else /* Local symbol.  */
      keep = (strip_symbols != STRIP_UNNEEDED && (discard_locals != LOCALS_ALL && (discard_locals != LOCALS_START_L || !bfd_is_local_label(abfd, sym))));
    ...
    if (keep && !(flags & BSF_KEEP) && is_specified_symbol(name, strip_unneeded_htab))
      keep = FALSE;

    if (!keep && ((keep_file_symbols && (flags & BSF_FILE)) || is_specified_symbol(name, keep_specific_htab)))
      keep = TRUE;
    ...
    if (keep)
    {
      if ((flags & BSF_GLOBAL) != 0 && (weaken || is_specified_symbol(name, weaken_specific_htab)))
      {
        sym->flags &= ~BSF_GLOBAL;
        sym->flags |= BSF_WEAK;
      }

      if (!undefined && (flags & (BSF_GLOBAL | BSF_WEAK)) && (is_specified_symbol(name, localize_specific_htab) || (htab_elements(keepglobal_specific_htab) != 0 && !is_specified_symbol(name, keepglobal_specific_htab)) || (localize_hidden && is_hidden_symbol(sym))))
      {
        sym->flags &= ~(BSF_GLOBAL | BSF_WEAK);
        sym->flags |= BSF_LOCAL;
      }

      if (!undefined && (flags & BSF_LOCAL) && is_specified_symbol(name, globalize_specific_htab))
      {
        sym->flags &= ~BSF_LOCAL;
        sym->flags |= BSF_GLOBAL;
      }
...
lookup_sym_redefinition(const char *source)
{
  ...
  struct redefine_node *redef_node = (struct redefine_node *)htab_find(redefine_specific_htab, &key_node);
...
add_redefine_and_check(const char *cause, const char *source,
                       const char *target)
{
  struct redefine_node *new_node = (struct redefine_node *)xmalloc(sizeof(struct redefine_node));
...
copy_unknown_object(bfd *ibfd, bfd *obfd)
{
  ...
  while (ncopied < size)
  {
    ...
    if (bfd_bwrite(cbuf, (bfd_size_type)tocopy, obfd) != (bfd_size_type)tocopy)
    {
...
merge_gnu_build_notes(bfd *abfd, asection *sec, bfd_size_type size, bfd_byte *contents)
{
 ...
 cannot safely merge it.  */
  ...
  while (remain >= 12)
  {
    ...
    if (pnote->note.namesz > 2 && pnote->note.namedata[0] == '$' && pnote->note.namedata[1] == GNU_BUILD_ATTRIBUTE_VERSION && pnote->note.namedata[2] == '1')
      ++version_1_seen;
    else if (pnote->note.namesz > 4 && pnote->note.namedata[0] == 'G' && pnote->note.namedata[1] == 'A' && pnote->note.namedata[2] == '$' && pnote->note.namedata[3] == GNU_BUILD_ATTRIBUTE_VERSION)
    {
  ...
  if ((version_1_seen > 0 && version_2_seen > 0) || (version_1_seen > 0 && version_3_seen > 0) || (version_2_seen > 0 && version_3_seen > 0))
  {
 ...
 Clear the type field of any found - but do not delete them just yet.  */
    for (back = pnote - 1; back >= pnotes; back--)
    {
      ...
      if (prev_open_with_range == NULL && back->note.descsz > 0 && !is_func_note(back))
      ...
      if (back_type == GNU_BUILD_ATTRIBUTE_STACK_SIZE)
      {
        ...
        name = (unsigned char *)back->note.namedata + val_start;
        while (name < (unsigned char *)back->note.namedata + back->note.namesz)
        {
          byte = back_val & 0xff;
          *name++ = byte;
          if (back_val == 0)
      ...
      if (back->note.namesz == pnote->note.namesz && memcmp(back->note.namedata, pnote->note.namedata, back->note.namesz) == 0 && !gap_exists(back, pnote))
      {
        duplicate_found = TRUE;
        pnote->note.type = 0;
     ...
     move the range into the note.  */
      ...
      {
        if (pnote->note.descsz == 0 && prev_open_with_range != NULL && prev_open_with_range->note.type == 0)
...
copy_object(bfd *ibfd, bfd *obfd, const bfd_arch_info_type *input_arch)
{
  ...
  if (ibfd->xvec->byteorder != obfd->xvec->byteorder && ibfd->xvec->byteorder != BFD_ENDIAN_UNKNOWN && obfd->xvec->byteorder != BFD_ENDIAN_UNKNOWN)
  {
 ...
 may need to tidy temporary files.  */
  ...
  if (input_arch)
  {
    if (bfd_get_arch_info(ibfd) == NULL || bfd_get_arch_info(ibfd)->arch == bfd_arch_unknown)
    {
  ...
  if (!bfd_set_arch_mach(obfd, iarch, imach) && (ibfd->target_defaulted || bfd_get_arch(ibfd) != bfd_get_arch(obfd)))
  {
  ...
  if (add_sections != NULL)
  {
    ...
    for (padd = add_sections; padd != NULL; padd = padd->next)
    {
     ...
     because we set the flags.  */
        padd->section = bfd_make_section_with_flags(obfd, padd->name, flags | SEC_LINKER_CREATED);
      ...
      if (pset != NULL && !bfd_set_section_vma(obfd, padd->section, pset->vma_val))
      {
        bfd_nonfatal_message(NULL, obfd, padd->section, NULL);
        return FALSE;
      }
      ...
      if (pset != NULL)
      {
        ...
        if (!bfd_set_section_alignment(obfd, padd->section,
 ...
 section size first, before its contents are ready.  */
    ...
    if (osec && is_merged_note_section(ibfd, osec))
    {
      ...
      else
      {
        ...
        {
          if (osec->output_section == NULL || !bfd_set_section_size(obfd, osec->output_section, merged_size))
          {
    ...
    else
    {
     ...
     highest VMA and start the debuglink section after that one.  */
        for (osec = obfd->sections, highest_section = NULL;
             osec != NULL;
             osec = osec->next)
          if (osec->vma > 0 && (highest_section == NULL || osec->vma > highest_section->vma))
 ...
 section.  */
  ...
  if (gnu_debuglink_filename != NULL)
  {
    if (!bfd_fill_in_gnu_debuglink_section(obfd, gnu_debuglink_section, gnu_debuglink_filename))
    {
      bfd_nonfatal_message(NULL, obfd, NULL,
...
set_long_section_mode(bfd *output_bfd, bfd *input_bfd, enum long_section_name_handling style)
{
  ...
  {
    if (style == KEEP && bfd_get_flavour(input_bfd) == bfd_target_coff_flavour)
                   ...
                   flagword flags)
{
  ...
  for (srename = section_rename_list; srename != NULL; srename = srename->next)
    if (strcmp(srename->old_name, old_name) == 0)
    {
      ...
      if (strcmp(srename->new_name, new_name) == 0 && srename->flags == flags)
        return;
...
setup_section(bfd *ibfd, sec_ptr isection, void *obfdarg)
{
  ...
  else if (strip_symbols == STRIP_NONDEBUG && (flags & (SEC_ALLOC | SEC_GROUP)) != 0 && !is_nondebug_keep_contents_section(ibfd, isection))
  {
...
copy_relocations_in_section(bfd *ibfd, sec_ptr isection, void *obfdarg)
{
  ...
  if (bfd_get_format(obfd) == bfd_core || strip_symbols == STRIP_NONDWO || discard_relocations(ibfd, isection))
    relsize = 0;
  else
  {
...
copy_section(bfd *ibfd, sec_ptr isection, void *obfdarg)
{
  ...
  if (bfd_get_section_flags(ibfd, isection) & SEC_HAS_CONTENTS && bfd_get_section_flags(obfd, osection) & SEC_HAS_CONTENTS)
  {
    bfd_byte *memhunk = NULL;

    if (!bfd_get_full_section_contents(ibfd, isection, &memhunk) || !bfd_convert_section_contents(ibfd, isection, obfd,
                                                                                                  &memhunk, &size))
    {
      status = 1;
      bfd_nonfatal_message(NULL, ibfd, isection, NULL);
...
compare_section_lma(const void *arg1, const void *arg2)
{
  ...
  flags1 = (*sec1)->flags;
  flags2 = (*sec2)->flags;
  if ((flags1 & SEC_HAS_CONTENTS) == 0 || (flags1 & SEC_LOAD) == 0)
  {
    if ((flags2 & SEC_HAS_CONTENTS) != 0 && (flags2 & SEC_LOAD) != 0)
      return -1;
  }
  else
  {
...
mark_symbols_used_in_relocations(bfd *ibfd, sec_ptr isection, void *symbolsarg)
{
  ...
  for (i = 0; i < relcount; i++)
  {
    ...
    if (relpp[i]->sym_ptr_ptr != NULL && *relpp[i]->sym_ptr_ptr != NULL && *relpp[i]->sym_ptr_ptr != bfd_com_section_ptr->symbol && *relpp[i]->sym_ptr_ptr != bfd_abs_section_ptr->symbol && *relpp[i]->sym_ptr_ptr != bfd_und_section_ptr->symbol)
...
{
  if (bfd_get_flavour(obfd) == bfd_target_coff_flavour || bfd_get_flavour(obfd) == bfd_target_elf_flavour)
  {
    ...
    if (stabsec == NULL || stabstrsec == NULL || !bfd_set_section_size(obfd, stabsec, symsize) || !bfd_set_section_size(obfd, stabstrsec, stringsize) || !bfd_set_section_alignment(obfd, stabsec, 2) || !bfd_set_section_alignment(obfd, stabstrsec, 0))
    {
      bfd_nonfatal_message(NULL, obfd, NULL,
 ...
 setting out of this function.  */
    if (!bfd_set_section_contents(obfd, stabsec, syms, 0, symsize) || !bfd_set_section_contents(obfd, stabstrsec, strings, 0,
                                                                                                stringsize))
    {
      bfd_nonfatal_message(NULL, obfd, NULL,
...
strip_main(int argc, char *argv[])
{
  ...
  if (strip_symbols == STRIP_UNDEF && discard_locals == LOCALS_UNDEF && htab_elements(strip_specific_htab) == 0)
    strip_symbols = STRIP_ALL;
  ...
  i = optind;
  if (i == argc || (output_file != NULL && (i + 1) < argc))
    strip_usage(stderr, 1);
  ...
  for (; i < argc; i++)
  {
    ...
    if (output_file == NULL || filename_cmp(argv[i], output_file) == 0)
...
copy_main(int argc, char *argv[])
{
  ...
  if (input_target != NULL && strncmp(input_target, "efi-", 4) == 0)
  {
    char *efi;

    efi = xstrdup(output_target + 4);
    if (strncmp(efi, "bsdrv-", 6) == 0 || strncmp(efi, "rtdrv-", 6) == 0)
      efi += 2;
  ...
  if (output_target != NULL && strncmp(output_target, "efi-", 4) == 0)
  {
    char *efi;
  ...
  if (output_filename == NULL || filename_cmp(input_filename, output_filename) == 0)

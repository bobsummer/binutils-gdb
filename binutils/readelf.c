		 ...
		 const char *reason)
{
	...
	if (amt > filedata->file_size || offset + archive_file_offset + amt > filedata->file_size)
	{
		if (reason)
...
parse_args(Filedata *filedata, int argc, char **argv)
{
	...
	if (!do_dynamic && !do_syms && !do_reloc && !do_unwind && !do_sections && !do_segments && !do_header && !do_dump && !do_version && !do_histogram && !do_debugging && !do_arch && !do_notes && !do_section_groups && !do_archive_index && !do_dyn_syms)
		usage(stderr);
}
...
process_file_header(Filedata *filedata)
{
	...
	if (do_header)
	{
		...
		else if (header->e_shstrndx != SHN_UNDEF && header->e_shstrndx >= header->e_shnum)
		...
		case PT_PHDR:
			...
			if (filedata->file_header.e_machine != EM_PARISC)
			{
				...
				for (j = 1; j < filedata->file_header.e_phnum; j++)
					if (filedata->program_headers[j].p_vaddr <= segment->p_vaddr && (filedata->program_headers[j].p_vaddr + filedata->program_headers[j].p_memsz) >= (segment->p_vaddr + segment->p_filesz))
						break;
...
#define CHECK_ENTSIZE(section, i, type)                             \
		 ...
		 i++, section++)
	{
		...
		case SHT_REL:
		case SHT_RELA:
			if (section->sh_link < 1 || section->sh_link >= filedata->file_header.e_shnum || (filedata->section_headers[section->sh_link].sh_type != SHT_SYMTAB && filedata->section_headers[section->sh_link].sh_type != SHT_DYNSYM))
		...
		case SHT_GNU_verdef:
		case SHT_GNU_LIBLIST:
			if (section->sh_link < 1 || section->sh_link >= filedata->file_header.e_shnum || filedata->section_headers[section->sh_link].sh_type != SHT_STRTAB)
	 ...
	 section.  */
	  if (section->sh_type < SHT_LOOS && section->sh_link != 0)
		...
		case SHT_REL:
		case SHT_RELA:
			if (section->sh_info < 1 || section->sh_info >= filedata->file_header.e_shnum || (filedata->section_headers[section->sh_info].sh_type != SHT_PROGBITS && filedata->section_headers[section->sh_info].sh_type != SHT_NOBITS && filedata->section_headers[section->sh_info].sh_type != SHT_NOTE && filedata->section_headers[section->sh_info].sh_type != SHT_INIT_ARRAY
																							  ...
																							  && filedata->section_headers[section->sh_info].sh_type < SHT_LOOS))
			{
				if (section->sh_info == 0 && (filedata->file_header.e_type == ET_EXEC || filedata->file_header.e_type == ET_DYN
											 ...
											 they have been left in for paranoia's sake.  */
											  || streq(SECTION_NAME(section), ".rel.dyn") || streq(SECTION_NAME(section), ".rela.dyn")))
...
decode_x86_isa(unsigned int bitmask)
{
	while (bitmask)
	{
		...
		switch (bit)
		{
		case GNU_PROPERTY_X86_ISA_1_486:
			printf("i486");
			break;
		case GNU_PROPERTY_X86_ISA_1_586:
			printf("586");
			break;
		case GNU_PROPERTY_X86_ISA_1_686:
			printf("686");
			break;
